...
from capsules.capsule.src.models.PackageModel import TrainExecutor,PackageModel,ConfigExecutor,TrainResponse,TrainOutputs,ImageData,OutputData,PackageConfigs
...
class Train(Capsule):
    ...
    def run(self):
            #status=self.train()
            status=True
            imageList = ImageData(name="Imagedata", value=[str(status)])
